...
if __name__ == '__main__':
    ...
    word_list= ["TRANSFORM","DIGITAL","HANGMAN","FORTRESS","ACADEMY","TECHNOLOGY","CAREER"]
    ...
    for c in ascii_uppercase:
        ...
        let_butt=ctk.CTkButton(master=root, text=c ,command=lambda c=c: guess(c), font=('Helvetica', 18), width=40, height=40).grid(row=1+n//9,column=n%9)
        n+=1

    re_butt=ctk.CTkButton(master=root, text="Restart", command=lambda:newGame(),  width=40, height=40,font=("Helvetica" ,10, "bold")).grid(row=3, column=8)
